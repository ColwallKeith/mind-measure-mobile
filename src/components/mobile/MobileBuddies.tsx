...
export function BuddyScreen() {
  ...
  // Prevent iOS zoom on input focus when add form is shown
  useEffect(() => {
    ...
    style.textContent = `
      /* Prevent zoom on input focus - Buddy page */
    ...
    console.log('🔧 Applied buddy page zoom prevention');
    // Cleanup when form is hidden or component unmounts
    return () => {
      ...
      console.log('🔄 Restored viewport settings after buddy form');
    };
  }, [showAddForm]);
  const handleAddBuddy = () => {
    if (formData.name && formData.email && formData.phone) {
      ...
      // Force viewport reset before hiding form
      const viewport = document.querySelector('meta[name="viewport"]');
      if (viewport) {
        viewport.setAttribute('content', 'width=device-width, initial-scale=1.0, viewport-fit=cover');
      }
...
export const MobileBuddies = BuddyScreen;